...
def glowint(i, mob, ocity, color=TEAL):
        glow_group = VGroup()
        
        for idx in range(60):
            # Calculate opacity with proper bounds
            opacity = max(0, (0.2 - idx/300) * ocity)
            
            # Create Integer mobject (requires Manim v0.17.0+)
            # For older versions, use Text or Tex
            new_i = Integer(
                i,
                color=color
            ).move_to(mob.get_center()).scale(idx/10)
            
            # Set fill properties
            new_i.set_fill(color=color, opacity=opacity)
            new_i.set_stroke(width=0)  # Remove stroke for glow effect
            
            glow_group.add(new_i)
